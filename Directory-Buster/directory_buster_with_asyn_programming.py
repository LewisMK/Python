...
def get_args(*args, **kwargs):
    ...
    parser.add_argument('-u', '--url', dest="target", help="The target URL to attack (include http or https)")
...
class Dir_bruteforcer():
    ...
    def print_banner(self):
            ...
            print("-"*80)
            print(colored(f"Directory and file bruteforcer starting at {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}", 'cyan', attrs=['bold']))
            print("-"*80)
            print(colored("[*] Target Url".ljust(20, " "),'light_red'), ":", f"{self.target}")
            print(colored("[*] Wordlist".ljust(20, " "),'light_red'), ":", f"{self.wordlist}")
            
            if self.headers:
                print(colored("[*] Headers".ljust(20, " "),'light_red'), ":", f"{headers}")
                
            if self.extensions:
                print(colored("[*] Extensions".ljust(20, " "),'light_red'), ":", f"{self.extensions}")
                
            if self.outputfile:
                print(colored("[*] Output File".ljust(20, " "),'light_red'), ":", f"{self.outputfile}")
                
            if self.match_size:
                print(colored("[*] Match Res size".ljust(20, " "), 'light_red'),":", f"{self.match_size}")
                
            if self.match_codes or self.filter_codes:
                if self.match_codes:
                    print(colored("[*] Match Codes".ljust(20, " "),'light_red'),":", f"{self.match_codes}")
                if self.filter_codes:
                    print(colored("[*] Filter Codes".ljust(20, " "), 'light_red'),":", f"{self.filter_codes}")
            else:
                print(colored("[*] Status Codes".ljust(20, " "),'light_red'), ":", f"All Status Codes")

            if self.filter_size:
                print(colored("[*] Filter Response Size".ljust(20, " "), 'light_red'),":", f"{self.filter_size}")
        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=self.headers) as session:
                ...
                for dir in dirs:
                    ...
                    tasks.append(asyncio.create_task(self.brute_dir(word=dir, session = session))) 
            ...
            print(f'An error occured during the HTTP request: {str(err)}')
            ...
            print(f"An unexpected erro occured:he {err}")
            exit(1)
...
if __name__ == "__main__":
    ...
    bruteforcer = Dir_bruteforcer(target, wordlist, extensions, redirection, headers, match_codes, match_size, filter_codes, filter_size, output, hide_title)
